...
public class SocketServer {
	...
	public SeOrderServiceImpl seOrderServiceImpl;

	public SocketServer(SeOrderServiceImpl ss) {
		seOrderServiceImpl =ss;
	}
	...
	public void run() throws IOException {
		...
		try {
			...
			while (true) {
				...
				try {
					new ServerOne(s,seOrderServiceImpl);
				} catch (IOException e) {
					s.close();
				}
			}
...
class ServerOne extends Thread {
	...
	public ServerOne(Socket s,SeOrderServiceImpl ss) throws IOException {
		this.s = s;
		in=s.getInputStream();
			...
			while (true) {
				...
				if("55550601".equals(commandStr)){
					...
					if(operResult.equals("00")){
                        seOrderServiceImpl.upgradeLockCron(new String (manName,"UTF-16LE"));
                    }
